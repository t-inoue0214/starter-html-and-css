...
このコースを修了すると、業務で発生する HTML/CSS の修正や新規コーディングを自走して行えるレベルを目指します。
...
- `Github`の`Codespaces`を利用します。`Codespaces`は設定によってはコストがかかるものなので [Codespace の利用上の注意](./CODE_SPACES_SERVICE.md) はよく確認してください。
- コストをかけないためにも、セキュリティの意味でも、使い終わったら [停止方法](./CODE_SPACES_SERVICE.md#3-停止方法) に従って停止することを推奨します。
...
1. **環境の準備を待つ**
    - ブラウザでVS Codeが起動します。
    - 初回はVS Code拡張機能（Prettier・日本語化パック）のセットアップのために1〜2分ほどかかります。
...
1. **ブラウザでプレビューを開く**
    - 画面右下に通知が表示されたら、**「ブラウザーで開く」** をクリックします。
    - 通知が消えてしまった場合は、VS Code 下部の「ポート」タブを開き、「転送されたアドレス」の URL をクリックしてください。
...
| [08_css_design_basics](./08_css_design_basics/README.md) | デザインの基礎 | タイポグラフィ・色・余白・視覚的階層・CSS 変数（デザイントークン）を学び、デザイナーと協業するための基礎知識を習得します。 |
| [09_accessibility](./09_accessibility/README.md) | アクセシビリティの基礎 | 色のコントラスト比・主要 ARIA 属性・キーボード操作・スクリーンリーダーによる確認方法を学習します。 |
| [10_css_responsive](./10_css_responsive/README.md) | レスポンシブデザイン | メディアクエリ・モバイルファーストの考え方を学び、画面幅に応じてレイアウトが変わるページを実装します。 |
...
├── assets/                <-- 画像などのリソース
├── README.md              <-- このファイル
├── CODE_SPACES_SERVICE.md <-- Codespaces の利用上の注意
